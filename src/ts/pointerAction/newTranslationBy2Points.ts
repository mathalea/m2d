...
export function newTranslationBy2Points (clickedElements: ClickedElements) {
  ...
  for (const e of clickedElements.all) {
    if (figure.selectedElements.length === 0 && e instanceof Point) {
      ...
      A = figure.selectedElements[0] as Point
      B = figure.selectedElements[1] as Point
      const v = new VectorByPoints(A, B)
      if (e instanceof Circle || e instanceof Line || e instanceof Polygon) {
        M = e.translationVector(v)
        if (M && figure.pointerSetOptions.color) M.color = figure.pointerSetOptions.color
        if (M && figure.pointerSetOptions.thickness) M.thickness = figure.pointerSetOptions.thickness
        if (M && figure.pointerSetOptions.dashed !== undefined) M.dashed = figure.pointerSetOptions.dashed
...
export function actionNewTranslationBy2PointseMessage (figure: Figure) {
