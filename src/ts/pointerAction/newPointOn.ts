...
export function newPointOn (figure: Figure, pointerX: number, pointerY: number) {
  for (const e of figure.set) {
    if (e instanceof Segment && e.distancePointer(pointerX, pointerY) * figure.pixelsPerUnit < 15) {
      ...
      if ((pointerX < e.A.x && e.A.x < e.B.x) || (pointerX > e.A.x && e.A.x > e.B.x)) d *= -1
      else if ((pointerY < e.A.y && e.A.y < e.B.y) || (pointerY > e.A.y && e.A.y > e.B.y)) d *= -1
...
export function actionNewPointOnMessage (figure: Figure) {
  ...
  } else figure.displayMessage('Cliquer sur l\'objet où placer le point')
}
