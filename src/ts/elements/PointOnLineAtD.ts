...
export class PointOnLineAtD extends Point {
  ...
  constructor (L: Segment, d: number, { style = 'x', size = 0.15, thickness = 3, color = 'Gray', dragable = false, temp = false }: {length?: number, k?: number} & PointOptions = {}) {
    const M = new PointByHomothetie(L.ends[1], L.ends[0], d / distance(L.ends[0], L.ends[1]), { temp: true })
    super(L.parentFigure, M.x, M.y, { style, size, thickness, color, dragable, temp })
    this.x = M.x
    this.y = M.y
    this.line = L
    this.d = d
